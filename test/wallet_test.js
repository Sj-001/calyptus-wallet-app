...
const SharedWallet = require("../artifacts/contracts/SharedWallet.sol/SharedWallet.json");
...
describe("WalletCreator", () => {
  ...
  describe("SharedWallet", () => {
    ...
    it("should not allow non  beneficiary address to spend funds from contract", async () => {
      await expect(
        contract.connect(addr1).withdraw(ethers.utils.parseEther("2"))
      ).to.be.reverted;
      await expect(
    ...
    it("should not allow beneficiary to spend more than the limit", async () => {
      await expect(
        contract.connect(addr2).withdraw(ethers.utils.parseEther("4"))
      ).to.be.reverted;
      await expect(
        contract
          .connect(addr2)
    ...
    it("should not allow to spend funds more than the contract balance", async () => {
      await expect(contract.withdraw(ethers.utils.parseEther("500"))).to.be
        .reverted;
    });
  });
});
